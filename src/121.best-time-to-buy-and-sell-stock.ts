...
var maxProfit = function(prices: number[]) {
  ...
  while(sellPointer <= prices.length - 1) {
    while(prices[sellPointer + 1] && prices[sellPointer + 1] > prices[sellPointer]) sellPointer++
    while(buyPointer + 1 < sellPointer && prices[buyPointer + 1] < prices[buyPointer]) buyPointer++
    const delta = prices[sellPointer] - prices[buyPointer]  
    if(delta > result) result = delta  

    sellPointer++
    if(prices[buyPointer] > prices[sellPointer]) buyPointer = sellPointer++ 
  }
