 ...
 * @return {string[][]}
 */
var findLadders = function(beginWord: string, endWord: string, wordList: string[]): string[][] {
  let results: string[][] = []
  const travel = (beginWord: string, endWord: string, wordList: string[], result: string[]) => {
    for(let p = 0; p < beginWord.length; p++) {
      ...
      for( let i = 0; i < wordList.length; i++ ) {
        ...
        if(except === wordExcept) {
          ...
          newList.splice(i, 1)
          if(word === endWord) return results.push(result)
...
console.log(findLadders("qa",
  "sq",
["si","go","se","cm","so","ph","mt","db","mb","sb","kr","ln","tm","le","av","sm","ar","ci","ca","br","ti","ba","to","ra","fa","yo","ow","sn","ya","cr","po","fe","ho","ma","re","or","rn","au","ur","rh","sr","tc","lt","lo","as","fr","nb","yb","if","pb","ge","th","pm","rb","sh","co","ga","li","ha","hz","no","bi","di","hi","qa","pi","os","uh","wm","an","me","mo","na","la","st","er","sc","ne","mn","mi","am","ex","pt","io","be","fm","ta","tb","ni","mr","pa","he","lr","sq","ye"]).length)
