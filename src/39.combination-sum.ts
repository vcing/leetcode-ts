...
const arrayLoop = (current: number[], length: number, arr: number[], target: number, result: number[][]) => {
  if (length > 1) {
    let subArr = arr
    if (current.length > 0) subArr = arr.slice(arr.indexOf(current[current.length - 1]))
    const start = arr.findIndex(n => n === current[current.length - 1])
    for (let i = start; i < arr.length; i++) arrayLoop([...current, arr[i]], length - 1, subArr, target, result)
  } else {
    const fixed = current.reduce((prev, curr) => prev + curr, 0)
    const start = arr.findIndex(n => n === current[current.length - 1])
    for (let i = start; i < arr.length; i++) {
      if (arr[i] + fixed === target) result.push([...current, arr[i]])
      else if (arr[i] + fixed > target) break
    }
  }
}
