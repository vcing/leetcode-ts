...
var minCut = function (s) {
    ...
    let result = Infinity;
    const sliceString = (start, length, slice) => {
        // console.log( start, length, slice )
        const end = start + length;
        if (length <= 1) {
            if (result > slice)
                result = slice - (length ? 0 : 1);
            return;
        }
        for (let l = length; length >= 1; l--) {
            ...
            if (originalTestResult)
                sliceString(start + l, length - l, slice + 1);
            if (reverseTestResult)
                sliceString(start, length - l, slice + 1);
            if (reverseTestResult || originalTestResult)
                return;
        }
    };
