...
var canCompleteCircuit = function(gas: number[], cost: number[]) {
  ...
  profit = profit.sort((a, b) => b.profit - a.profit) 
  for (let i = 0; i < profit.length; i++) {
    const p = profit[i]
    let left = 0
    let finish = false
    for( let i = 0, current = p.index; i < profit.length; i++, current = current + 1 < profit.length ? current + 1 : 0) {
      if( left + gas[current] < cost[current] ) break 
      left = left + gas[current] - cost[current]
      if(i === profit.length - 1) finish = true
    }
    if(finish) return p.index
  }
};
