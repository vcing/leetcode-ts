...
var restoreIpAddresses = function (s: string) {
  const result: string[] = []
  const backTracking = (current: number[], left: string) => {
    // console.log(current)
    if (current.length === 4 && left === '') return result.push(current.join('.'))
    ...
    for (let i = min; i <= max; i++) {
      ...
      if (number < 256) backTracking([...current, number], left.substr(i))
    }
  }
  backTracking([], s)
  return result
