 ...
 * @return {number[]}
 */
var spiralOrder = function (matrix: number[][], n = 0): number[] {
  ...
  if (width !== 1) result = result.concat(matrix.slice(n + 1, height - 1 - n).map(a => a[n]).reverse())
  // console.log(nextWidth, nextHeight, result)
  if (nextWidth > 1 && nextHeight > 1) return result.concat(spiralOrder(matrix, n + 1))
  else if (nextWidth === 1 && nextHeight > 0) return result.concat(matrix.slice(n + 1, height - 1 - n).map(a => a[width - n - 2]))
  else if (nextHeight === 1 && nextWidth > 0) return result.concat(matrix[n + 1].slice(n + 1, width - n - 1))
  else return result
};
