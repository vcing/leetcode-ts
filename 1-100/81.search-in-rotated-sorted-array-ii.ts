 ...
 * @return {boolean}
 */
var SRSA = function (nums: number[], target: number): boolean {
  nums = [...new Set(nums)]
  // console.log(nums)
  if (nums.length <= 1) return target === nums[0]
  if (nums.length <= 2) return target === nums[0] || target === nums[1]
  const findStart = () => {
    ...
    while (true) {
      ...
      if ((nums[result - 1] === undefined || nums[result] < nums[result - 1]) && (nums[result + 1] === undefined || nums[result] <= nums[result + 1])) return result
  ...
  const binarySearch = (nums: number[]): boolean => {
    const middle = Math.floor(nums.length / 2)
    if (nums.length <= 1) return target === nums[0]
    if (target === nums[middle]) return true
    if (target > nums[middle]) return binarySearch(nums.slice(middle + 1))
    if (target < nums[middle]) return binarySearch(nums.slice(0, middle))
  }
  ...
  if (target === nums[middle]) return true
    ...
    if (target > nums[last]) return false
